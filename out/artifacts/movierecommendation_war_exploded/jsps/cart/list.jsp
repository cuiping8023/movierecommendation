...
$(function(){
	...
	*/
	$(":checkbox[name=checkboxBtn]").click(function(){
		...
		var select = $(":checkbox[name=checkboxBtn][checked=true]").length;//获取所有被选中条目的个数
		if(all == select){//全都被选中了
			$("#selectAll").attr("checked",true);//勾选全选复选框
			setJieSuan(true);//让结算按钮有效
		}else if(select == 0){//都没有选中
...
function showTotal(){
	...
	*/
	$(":checkbox[name=checkboxBtn][checked=true]").each(function(){
		//获取复选框的值，即cartItemId
		var id = $(this).val();
		//再通过cartItemId找到小计元素，获取其文本
		var text = $("#" + id + "Subtotal").text();
		//累加计算
		 total += Number(text);	
	});
	//把总计显示在总计元素上
	$("#total").text(round(total,2));//round()函数作用把tobal保留两位小数
}
/*
 * 统一设置所有条目的复选按钮
 */
...
function setJieSuan(bool){
	if(bool){
		...
		$("#jiesuan").unbind("click");//撤销当前元素的所有click事件
	}else{
		$("#jiesuan").removeClass("jiesuan").addClass("kill");
		$("#jiesuan").click(function(){return false;});//取消链接
	}
 ...
 function jiesuan(){
	//1.获取所有被选择的条目的id，放到数组中
	 var cartItemIdArray = new Array();
		...
		//把数组的中toString()，然后赋值给表单的cartItemIds这个hidden
